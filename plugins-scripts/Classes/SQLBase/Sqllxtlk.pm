...
sub write_extcmd_file {
  ...
  printf CMDCMD "SET NULLS 'X___NULL___X';\n";
...
sub add_dbi_funcs {
  ...
  $self->SUPER::add_dbi_funcs();
  {
    ...
    *{'Monitoring::GLPlugin::DB::decode_password'} = \&{"Classes::SQLBase::Sqllxtlk::decode_password"};
...
__END__
    foreach my $line (split(/\n/, $output)) {
      ...
      if ($line =~ /^\s*$/) {
        ...
        my @result = ();
        foreach my $resultpart (@resultparts) {
          push(@result, substr($line, $resultpart->[0], $resultpart->[1]));
        }
        push(@rows, \@result);
      } else {
      }
